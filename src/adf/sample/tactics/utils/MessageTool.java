...
public class MessageTool {
  ...
  public MessageTool( ScenarioInfo scenarioInfo, DevelopData developData ) {
    ...
    this.estimatedMoveDistance = developData.getInteger(
        "sample.tactics.MessageTool.estimatedMoveDistance", 40000 );
            ...
            case BUILDING:
              ...
              if ( isOnFireOrWaterDameged( building ) ) {
                message = new MessageBuilding( true,
                    StandardMessagePriority.LOW, building );
              }
              break;
            case CIVILIAN:
              ...
              if ( isUnmovalCivilian( civilian ) ) {
                message = new MessageCivilian( true,
                    StandardMessagePriority.LOW, civilian );
              }
              break;
          }
  ...
  private double getAngle( Vector2D v1, Vector2D v2 ) {
    ...
    double angle = Math
        .acos( ((v1.getX() * v2.getX()) + (v1.getY() * v2.getY()))
  ...
  private int getDistance( double fromX, double fromY, double toX,
      double toY ) {
    double dx = toX - fromX;
    double dy = toY - fromY;
