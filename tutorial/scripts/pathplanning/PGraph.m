...
%   g.shortestpath(s, g, 'algorithm', 'Dijkstra')	shortest path from s to g without heuristics
        ...
        function [path,cost] = Astar(g, vstart, vgoal, varargin)
            ...
            while ~isempty(openSet)
                ...
                if vcurrent == vgoal
                    ...
                    while true
                        path = [p path];
                        p = came_from(p); % backtracking only works if we do it directly!
                        if p == 0
                            break;
                        end
                    end
...
%                         end
        ...
        % This is an additional to the original file PGraph.m for the 'Machine Learning to Rescue' workshop
        % See https://github.com/IntelligentRoboticsLab/Joint-Rescue-Forces/wiki
        %
        % The function is generalization of the original Astar function to Dijkstra, breadth and depth-first
        % The algorithm is selected by the parameter-option algorithm.
        %
        % Arnoud Visser, University of Amsterdam, June 2018
 
        function [path,cost] = shortestpath(g, vstart, vgoal, varargin)
            %PGraph.shortestpath path finding
            %
            % PATH = G.shortestpath(V1, V2) is the lowest cost path from vertex V1 to
            % vertex V2.  PATH is a list of vertices starting with V1 and ending
            % V2.
            %
            % [PATH,C] = G.shortestpath(V1, V2) as above but also returns the total cost
            % of traversing PATH.
            %
            % PATH = G.shortestpath(V1, V2, algorithm='Dijkstra') sorts the frontier
            % on the distance traveled so far (default option).
            %
            % PATH = G.shortestpath(V1, V2, algorithm='breadth-first') sorts the frontier
            % based on first-in, first-out.
            %
            % PATH = G.shortestpath(V1, V2, algorithm='Astar') sorts the frontier
            % on the distance traveled so far and an estimated distance still to go.
            %
            % Notes::
            % - Based on Peter Corke's orginal Astar implementation
            % - Implementation of the alternative algorithms was inspired by
            %   https://www.redblobgames.com/pathfinding/a-star/introduction.html
            %
            % See also PGraph.Astar, PGraph.goal, PGraph.path.
            
            
            opt.directed = false;
            opt.timer = false;
            opt.algorithm = {'Dijkstra', 'breadth-first', 'depth-first', 'Astar'};
            
            opt = tb_optparse(opt, varargin);
            
            switch opt.algorithm
            case 'Dijkstra'
                display 'Calculating shortestpath with Dijkstra algorithm';
                use_heuristics=false;
                use_optimal=true;
            case 'breadth-first'
                display 'Calculating shortestpath with breadth-first algorithm';
                use_gready=false;
                use_heuristics=false;
                use_optimal=false;
            case 'depth-first'
                display 'Calculating shortestpath with depth-first algorithm';
                use_gready=true;
                use_heuristics=false;
                use_optimal=false;
            case 'Astar'
                display 'Calculating shortestpath with Astar algorithm';
                use_heuristics=true;
                use_optimal=true;
            otherwise
                display 'unknown algorithm, choose between Dijkstra, Astar, breadth- or depth-first';
                display 'Calculating shortestpath with Dijkstra algorithm';
                opt.algorithm = 'Dijkstra';
                use_heuristics=false;
                use_optimal=true;
            end
            ...
            while ~isempty(openSet)
                ...
                if use_optimal
                    ...
                    if use_gready       
                        vcurrent = openSet(end); % depth-first
                    else
                        vcurrent = openSet(1); % breadth-first
                    end
                end
                
                if vcurrent == vgoal
                    % we have arrived! % Yet with breadth-first there could be multiple solutions.
                    if f_score(vcurrent) < cost % overwrite path and cost when better
                       p = vgoal;
                       while true
                           path = [p path];
                           p = came_from(p);
                       ...
                       cost = f_score(vcurrent)
                    else
                       sprintf('Path with length %f was not shorter than previous length %f',f_score(vcurrent),cost);
                    end
%                    if use_optimal || use_gready % sorting guarantees that once is enough
                    if use_optimal  % sorting guarantees that once is enough
                        if opt.timer
                            toc
                        end
                        return
                    else
			display 'Continue looking for a better path'
                    end
                end
