...
public class SampleTacticsFireBrigade extends TacticsFireBrigade {
  ...
  public void initialize( AgentInfo agentInfo, WorldInfo worldInfo,
    ...
    switch ( scenarioInfo.getMode() ) {
      ...
      case PRECOMPUTED:
        ...
        this.buildingDetector = moduleManager.getModule(
        ...
        this.actionFireFighting = moduleManager.getExtAction(
            ...
            "adf.sample.extaction.ActionFireFighting" );
  ...
  public Action think( AgentInfo agentInfo, WorldInfo worldInfo,
    ...
    Action action = this.actionFireFighting.setTarget( target ).calc()
        .getAction();
    if ( action != null ) {
      this.sendActionMessage( messageManager, agent, action );
      return action;
  ...
  private void sendActionMessage( MessageManager messageManager,
      FireBrigade agent, Action action ) {
    ...
    if ( actionIndex != -1 ) {
      messageManager.addMessage(
          new MessageFireBrigade( true, agent, actionIndex, target ) );
    }
  }
}
