...
// This is the Joint Rescue Force code for the Machine Learning workshop -
// developed April 2018
...
public class MATLABshortestPathPlanning extends PathPlanning {
  ...
  public MATLABshortestPathPlanning( AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData ) {
    ...
    System.out
        .println( "[" + this.getClass().getSimpleName() + "] starting up" );
    try {
  ...
  public PathPlanning precompute( PrecomputeData precomputeData ) {
    ...
    System.out.println(
        "[" + this.getClass().getSimpleName() + "] calling precompute" );
  ...
  private void matlab_init() {
    ...
    try {
      ...
      // Create empty table
      this.ml.eval(
          "nodeTable = table([],[],[],'VariableNames',{'ID', 'X','Y'});G=graph([],[],[],nodeTable);" );
      ...
      for ( Entity next : this.worldInfo.getEntitiesOfType(
        ...
        matlab_command = String.format(
            "G=addnode(G,table(\"%d\",%d,%d,'VariableNames',{'ID' 'X','Y'}));",
            next.getID().getValue(), loc.first(), loc.second() );
      ...
      for ( Entity next : this.worldInfo.getEntitiesOfType(
        ...
        if ( next instanceof Area ) {
          ...
          for ( EntityID neighbour : areaNeighbours ) {
            if ( next.getID().getValue() > neighbour.getValue() ) { // only do
                                                                    // it once
              matlab_command = String.format(
                  "G=addedge(G,find(G.Nodes.ID==\"%d\"), find(G.Nodes.ID==\"%d\"), %d);",
                  next.getID().getValue(), neighbour.getValue(),
                  this.worldInfo.getDistance( next.getID(), neighbour ) ); // by
                                                                           // name,
                                                                           // not
                                                                           // by
                                                                           // index
  ...
  public PathPlanning resume( PrecomputeData precomputeData ) {
    ...
    System.out
        .println( "[" + this.getClass().getSimpleName() + "] calling resume" );
    return this;
  }
  ...
  public PathPlanning calc() {
    ...
    System.out.println( "[" + this.getClass().getSimpleName()
        + "] planning from location with ID: " + this.from + " to "
        + this.targets );
    
    try {
      // Start MATLAB asynchronously
      // Future<MatlabEngine> eng = MatlabEngine.startMatlabAsync(); // already
      // done in constructor
      ...
      System.out.println( "[" + this.getClass().getSimpleName()
          + "] math_calc(): from:" + from_string );
      
      int target_length = this.targets.size();
      if ( target_length < 2 ) {
        System.out.println( "[" + this.getClass().getSimpleName()
            + "] math_calc(): only:" + this.targets );
      }
      String[] targetlist = new String[target_length];
      int t = 0;
      for ( Iterator<EntityID> iterator = this.targets.iterator(); iterator
          .hasNext(); ) {
        EntityID target = iterator.next();
        targetlist[t++] = String.valueOf( target.getValue() );
      }
      
      String[] mlOutput = this.ml.feval( "getPath", from_string, targetlist ); // calls
                                                                               // getpath.m
      ...
      for ( int i = mlOutput.length - 1; i >= 0; i-- ) {
        ...
        // System.out.println("[" + this.getClass().getSimpleName() + "]
        // math_calc(): executing: get_path() gave result: " + waypointS);
        
        EntityID waypoint_entity = new rescuecore2.worldmodel.EntityID(
            Integer.parseInt( waypointS ) );
    ...
    System.out
        .println( "[" + this.getClass().getSimpleName() + "] found a path:" );
